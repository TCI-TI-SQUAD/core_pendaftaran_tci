...
@section('page-name-header','Create Siswa')
            ...
            <div class="row">
                    ...
                    <div class="col-12 col-lg-9">
                        <div class="container">
                            <div class="row">
                                <div class="col-12">
                                    <form action="{{ route('admin.store.create.siswa') }}" method="POST" enctype="multipart/form-data" id="create-user">
                                        @csrf
                                        @method('POST')
                                        ...
                                        <label >Name</label>
                                        <input name="name" type="text" placeholder="Nama siswa" class="form-control @error('name') border border-danger @enderror" value="{{ old('name') }}">
                                        @error('name') <p class="p-0 m-0 text-danger"><small>{{ $errors->first('name') }}</small></p> @enderror

                                        <label class="mt-2">Username</label>
                                        <input name="username" type="text" value="{{ old('username') }}" placeholder="Username siswa" class="form-control @error('username') border border-danger @enderror">
                                        @error('username') <p class="p-0 m-0 text-danger"><small>{{ $errors->first('username') }}</small></p> @enderror

                                        <label class="mt-2">HSK</label>
                                        <select name="hsk" class="browser-default custom-select @error('hsk') border border-danger @enderror">
                                        ...
                                        <label class="mt-2">Instansi</label>
                                        <select name="status" onchange="hideInputSystem(this.value)" type="text" placeholder="Pilih status" class="browser-default custom-select @error('status') border border-danger @enderror" required>
                                            <option value="">Pilih Instansi Siswa</option>
                                        ...
                                        <input name="email" type="text" value="{{ old('email') }}" placeholder="E-mail Siswa" class="form-control @error('email') border border-danger @enderror">
                                        @error('email') <p class="p-0 m-0 text-danger"><small>{{ $errors->first('email') }}</small></p> @enderror

                                        <label class="mt-2">Phone Number</label>
                                        <input name="phone_number" type="tel" value="{{ old('phone_number') }}" placeholder="Nomor telephone siswa" class="form-control @error('phone_number') border border-danger @enderror" pattern="/(\+62)([0-9]*$)/" minlength="7" maxlength="15" >
                                        ...
                                        <input name="line" type="text" value="{{ old('line') }}" placeholder="ID line siswa" class="form-control @error('line') border border-danger @enderror">
                                        @error('line') <p class="p-0 m-0 text-danger"><small>{{ $errors->first('line') }}</small></p> @enderror

                                        <label class="mt-2">WA</label>
                                        <input name="wa" type="tel" value="{{ old('wa') }}" placeholder="Nomor WA siswa" class="form-control @error('wa') border border-danger @enderror" pattern="/(\+62)([0-9]*$)/" minlength="7" maxlength="15" >
                                        @error('wa') <p class="p-0 m-0 text-danger"><small>{{ $errors->first('wa') }}</small></p> @enderror


                                        <label class="mt-2">Alamat</label>
                                        <input name="alamat" type="text" value="{{ old('alamat') }}" placeholder="Alamat siswa" class="form-control @error('alamat') border border-danger @enderror">
                                        @error('alamat') <p class="p-0 m-0 text-danger"><small>{{ $errors->first('alamat') }}</small></p> @enderror

                                        <label class="mt-2">Kartu Identitas</label> 
                                        <div class="input-group my-3 @error('file_kartu_identitas') border border-danger @enderror">
                                            ...
                                            <div class="custom-file">
                                                ...
                                                <label class="custom-file-label" for="kartu_identitas">Pilih File Kartu Identitas</label>
                                            </div>
                                        </div>
                                        ...
                                        <div class="custom-control custom-radio">
                                            <input type="radio" class="custom-control-input" id="defaultGroupExample1" name="jenis_kartu_identitas" value="ktp" required @if(old("jenis_kartu_identitas") == "ktp") checked @endif>
                                            <label class="custom-control-label" for="defaultGroupExample1">KTP</label>
                                        </div>

                                        <!-- Group of default radios - option 2 -->
                                        <div class="custom-control custom-radio mt-2">
                                            <input type="radio" class="custom-control-input" id="defaultGroupExample2" name="jenis_kartu_identitas" value="nisn" required @if(old("jenis_kartu_identitas") == "nisn") checked @endif>
                                            <label class="custom-control-label" for="defaultGroupExample2">NISN</label>
                                        </div>

                                        <!-- Group of default radios - option 2 -->
                                        <div class="custom-control custom-radio mt-2">
                                            <input type="radio" class="custom-control-input" id="defaultGroupExample3" name="jenis_kartu_identitas" value="ktm" required @if(old("jenis_kartu_identitas") == "ktm") checked @endif>
                                            <label class="custom-control-label" for="defaultGroupExample3">KTM (Kartu Tanda Mahasiswa)</label>
                                        </div>

                                        <!-- Group of default radios - option 3 -->
                                        <div class="custom-control custom-radio mt-2">
                                            <input type="radio" class="custom-control-input" id="defaultGroupExample4" name="jenis_kartu_identitas" value="passport" required @if(old("jenis_kartu_identitas") == "passport") checked @endif>
                                        ...
                                        <input name="password" type="password" placeholder="Password siswa" class="form-control @error('password') border border-danger @enderror" required>
                                        ...
                                        <input name="password_confirmation" type="password" placeholder="Password confirmation siswa" class="form-control @error('password_confirmation') border border-danger @enderror" required>
                                        ...
                                        <select name="hak_akses" class="browser-default custom-select @error('hak_akses') border border-danger @enderror">
                                            ...
                                            <option value="aktif" @if(old("hak_akses") == "aktif") selected @endif>AKTIF</option>
                                            <option value="ban"@if(old("hak_akses") == "ban") selected @endif>BAN</option>
                                        </select>
...
<script>
    ...
    function peringatanAdmin(){
        ...
        '<ul class="text-left">'+
        '<li>Mohon pastikan semua data yang telah diinputkan bersifat benar</li>'+
        '<li>Fitur ini digunakan untuk meningkatkan <b>KUASA</b> admin dalam mengatur data siswa</li>'+
        '<li>Gunakan fitur dengan bijak agar data tetap <b>VALID</b> </li>'+
        '</ul>'
        ,
        icon:'warning',
        showDenyButton: true,
